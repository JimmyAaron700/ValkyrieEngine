...
* **🎭 半无头静默模式 (Semi-Headless Mode)**
    * 程序启动时支持用户动态选择“经典可视化”或“静默无头”运行策略。
    * 系统通过图形化界面完成人工验证码鉴权后，自动执行跨进程的 Cookies 与 Web Storage (LocalStorage/SessionStorage) 全维度状态迁移。
    * 无头浏览器实例在后台完美克隆登录状态，后续检索与提取流程全程不抢夺鼠标焦点，不弹窗，实现真正的免打扰。
...
* **🔄 数据清洗与序列化管线 (Data I/O Pipeline)**
    ...
    * 实时存档，核心循环每处理完一个项目编号，立即执行一次 Excel 序列化存档。
